...
\CFuncExternal{glViewport}) as large as possible.  The size of the graphics
...
\begin{Table}{3}
  \textC{void }\CFunc{icetDataReplicationGroup}\textC{(}&\textC{GLint}&\CArg{size}\textC{,} \\
  &\textC{const GLint *}&\CArg{processes}\quad\textC{);}
\end{Table}
...
which it belongs.
\begin{Table}{3}
  \textC{void }\CFunc{icetDataReplicationGroupColor}\textC{(}&\textC{GLint}&\CArg{color}\quad\textC{);}
\end{Table}
...
Whenever \CFunc{icetDrawFrame} is called, \IceT measures the amount of time
spent in the various tasks required for parallel rendering.  These timings
are stored in the \IceT state and can be retrieved with \CFunc{icetGet}.
The state variables containing these timing metrics (in seconds) are as
follows.

\begin{Description}[xxxxxxxx]
\item[\CEnum{ICET\_RENDER\_TIME}] The total time spent in the drawing
  callback during the last call to \CFunc{icetDrawFrame}.
\item[\CEnum{ICET\_BUFFER\_READ\_TIME}] The total time spent reading from
  OpenGL buffers during the last call to \CFunc{icetDrawFrame}.
...
\item[\CEnum{ICET\_BLEND\_TIME}]/\CEnum{ICET\_COMPARE\_TIME} The total time
  ...
  last call to \CFunc{icetDrawFrame}.  These two variables always return
  the same value.
\item[\CEnum{ICET\_RENDER\_TIME}] The total time spent in the drawing
  callback during the last call to \CFunc{icetDrawFrame}.
\item[\CEnum{ICET\_COMPOSITE\_TIME}] The total time spent in compositing
  during the last call to \CFunc{icetDrawFrame}.  Equal to
...
\IceT also keeps track of the number of times \CFunc{icetDrawFrame} has
been called.  This number is stored in \CEnum{ICET\_FRAME\_COUNT}.
