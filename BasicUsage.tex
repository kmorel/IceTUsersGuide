...
Most compositing engines will simply take a group of images and combine
them together.  This approach, however, can be unreasonable when
compositing the high resolution images on a large tiled display. Although
\IceT can accept a single rendered image when that is convenient, \IceT can
also manage rendering when it is problematic to create display-sized images
that are larger than any buffer rendering hardware can directly produce.

To manage these problematic cases, the \IceT algorithms deal with pieces of
the overall image.  The image pieces are created on demand.  As such, \IceT
may require the same geometry to be rendered multiple times in a single
frame.  \IceT provides the application with the most flexible way to define
the rendering process: with a \keyterm{drawing callback}.
...
It is also possible to use \IceT without callbacks, which is sometimes more
convenient for applications. Rendering without callbacks is discussed in
the following section on rendering.
...
\subsection{Rendering Without Callbacks}
\label{sec:Basic_Usage:Rendering:Without_Callbacks}

Sometimes it is inconvenient to structure an application such that it can
render images from within an \IceT callback. For this case, \IceT supports
a mechanism that allows rendering images completely independent of the
compositing done within the \IceT library. This is done by first rendering
images and then calling \CFunc{icetCompositeImage}.
...
directly to \CFunc{icetCompositeImage}.
...
Another very important consideration to note is that when compositing with
color blending (\CFunc{icetCompositeMode} is set to
\CEnum{ICET\_COMPOSITE\_MODE\_BLEND}), the color buffer must be rendered
with a black background in order for the composite to complete correctly. A
colored background can later be added using the \CArg{background\_color} as
described below.
...
arguments that specify the projection that was used during rendering. They
have the same meaning as the like arguments to \CFunc{icetDrawFrame}.
...
\CFunc{icetCompositeImage} returns an \CType{IceTImage} containing the
composited image displayed on this process.  If the process is not
displaying a tile, then the contents of the image is undefined.
...
\end{Table}
  ...
  \begin{Table}{4}
    \textC{const IceTUByte *}&\icetImageGetColorcub&\textC{(}\quad\textC{const }\CType{IceTImage}&\CArg{image}\quad\textC{)}; \\
    \textC{const IceTUInt *}&\icetImageGetColorcui&\textC{(}\quad\textC{const }\CType{IceTImage}&\CArg{image}\quad\textC{)}; \\
    \textC{const IceTFloat *}&\icetImageGetColorcf&\textC{(}\quad\textC{const }\CType{IceTImage}&\CArg{image}\quad\textC{)}; \\
    \\
    \textC{const IceTFloat *}&\icetImageGetDepthcf&\textC{(}\quad\textC{const }\CType{IceTImage}&\CArg{image}\quad\textC{)};
  \end{Table}
