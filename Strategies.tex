...
returns a composited image.  The \CType{IceTImage} type is opaque to
...
\begin{Table}{3}
  \textC{GLuint }\CFunc{icetCompressSubImage}\textC{(}&\textC{const }\CType{IceTImage}&\CArg{imageBuffer},\\
...
\begin{Table}{3}
  \textC{GLuint }\CFunc{icetDecompressImage}\textC{(}&\textC{const }\CType{IceTSparseImage}&\CArg{compressedBuffer},\\
...
the strategies directory) provies \CFunc{icetAddSentBytes}.
...
\begin{Table}{3}
  \textC{void }\CFunc{ICET\_COMM\_WAITANY}\textC{(}&\textC{int}&\CArg{count},\\
...
\index{manpage:icetRenderTransferFullImages}
\begin{Table}{3}
  \textC{void }\CFunc{icetRenderTransferFullImages}\textC{(}&\CType{IceTImage}&\CArg{imageBuffer}\textC{,}\\
...
\CFunc{icetRenderTransferFullImage} renders all the tiles that are
...
can be determined with the \CFunc{icetCompressedBufferSize} macro in
...
\index{manpage:icetSendRecvLargeMessages}
...
\begin{Table}{3}
  \textC{void }\CFunc{icetSendRecvLargeMessages}\textC{(}&\textC{GLint}&\CArg{numMessagesSend}\textC{,}\\
...
message (in bytes) are returned.  The \CArg{generateDataFunc} will not be
